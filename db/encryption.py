...
def make_auth_token(username: str,
    ...
    """Конкатенирует `username` и `password` с разделителем `USERNAME_AND_PASSWORD_SEP`,
    после чего преобразует строку в BasicToken.
    """
...
def decode_auth_token(auth_token: str) -> tuple[str, str]:
    """Дешифрует BasicToken и возвращает имя пользователя и пароль."""
